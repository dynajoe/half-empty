...
(function() {
      ...
      if (index % 4 !== 0) {
        ...
        newText += chunk.replace(twttr.txt.regexen.autoLinkUsernamesOrLists, function(match, before, at, user, slashListname, offset, chunk) {
          ...
          if (slashListname && !options.suppressLists) {
            ...
            return stringSupplant("#{before}<a target=\"_blank\" class=\"#{urlClass} #{listClass}\" href=\"#{listUrlBase}#{list}\"#{extraHtml}>#{at}#{preChunk}#{chunk}#{postChunk}</a>", d);
          } else {
            if (after && after.match(twttr.txt.regexen.endScreenNameMatch)) {
              ...
              return stringSupplant("#{before}<a target=\"_blank\" class=\"#{urlClass} #{usernameClass}\" #{dataScreenName}href=\"#{usernameUrlBase}#{chunk}\"#{extraHtml}>#{at}#{preChunk}#{chunk}#{postChunk}</a>", d);
            }
          }
        });
      }
  ...
  twttr.txt.autoLinkHashtags = function(text, options) {
    ...
    return text.replace(twttr.txt.regexen.autoLinkHashtags, function(match, before, hash, text, offset, chunk) {
      ...
      return stringSupplant("#{before}<a href=\"#{hashtagUrlBase}#{text}\" target=\"_blank\" title=\"##{text}\" class=\"#{urlClass} #{hashtagClass}\"#{extraHtml}>#{hash}#{preText}#{text}#{postText}</a>", d);
    });
  };
  ...
  twttr.txt.autoLinkUrlsCustom = function(text, options) {
    ...
    return text.replace(twttr.txt.regexen.extractUrl, function(match, all, before, url, protocol, port, domain, path, queryString) {
      ...
      if (protocol) {
        ...
        return stringSupplant("#{before}<a target=\"_blank\" href=\"#{url}\"#{htmlAttrs}>#{displayUrl}</a>#{after}", d);
      } else {
        return all;
      }
    });
